...
(function () {
    ...
    /*
	 * The method dw.JSON.stringify is adapted from Douglas Crockford's JSON
...
var exportCSV = function exportToCsv(filename, rows) {
...
//		console.log("data--------------------"+JSON.stringify(data),"columns-------------" +JSON.stringify(columns));
	...
	var draw_controls = function(){
		...
		if(vtable.page_length() < vtable.schema_table().rows()){
			...
			var prev = dw.jq('span').addClass('next').addClass('paginate_button').append('prev').click(function(){
				...
				var download = dw.jq('span').addClass('paginate_button').append('Download CSV').click(function(filename, rows){
					exportCSV('Export File',data);
				})
		}
		...
		controls.append(tablesize).append(prev).append(next).append(download)
